...
export default function Navbar() {
  ...
  return (
    <div
      className={`visible ${show && "nav-hidden"} shadow-lg bg-[#81D6E3] 
     z-50`}
    >
      <div className="w-full flex items-center justify-between px-3 md:px-24 py-3">
        <div>
          <Link to="/">
            <h1 className="text-2xl text-slate-900 font-poppins drop-shadow-2xl	font-bold">
              Hanzala Shaikh
            </h1>
          </Link>
        </div>
            ...
            {navLinks.map((navItem) => (
              <li className="mx-4" key={navItem.title}>
                <NavLink
                  to={navItem.link}
                  style={activeLink}
                  className="text-amber-700 hover:text-slate-600 duration-300"
                >
                  {navItem.title}
                </NavLink>
              </li>
            ...
            >
              ...
              <div className="text-center">
                <p className="text-neutral">
                  &copy; Copyright 2024, Hanzala Shaikh. All Rights Reserved
                </p>
              </div>
            </Drawer>
          </div>
